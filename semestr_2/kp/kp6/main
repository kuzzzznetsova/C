/*
Выяснить, в какой группе разность между максимальным и минимальным средним баллом студентов максимальна.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int read_bin(FILE *in, person *stud){
    ...
    fread(stud->fam, sizeof(char), 22, in);
    fread(stud->init, sizeof(char), 9, in);
    fread(stud->sex, sizeof(char), 7, in);
    fread(&stud->group, sizeof(int), 4, in);
...
void find_result_group(person *stud, FILE *in) {
	...
	while(read_bin(in, stud)) {
		...
		if (current_group->min == -1 && current_group->max == -1)
		{
